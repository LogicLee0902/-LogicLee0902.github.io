...
表达式的 **静态类型** 在编译期已知, **动态类型** 在运行时可知. 非引用和指针的变量, 其静态类型与动态类型相同, 但是 `Base`
...
`Disc_quote::net_price` 使用, 相当于提供了 **默认实现**.
...
**"名字查找优先于类型查找"** 的规则导致, 如果 `D` 重载了 `B` 的某个函数, 那么 `D` 要把所有同名函数都覆盖一遍, 否则一些
`B` 的同名函数会被隐藏.
