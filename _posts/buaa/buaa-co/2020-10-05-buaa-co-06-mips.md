...
- `.ascii`: 以字节为单位存储字符串, 不包括 `\n`
- `.asciiz`: 以字节为单位存储字符串, 自动添加 `\n`
...
- `addu rt, rs, immediate`: 加一个立即数 (不检测溢出)
...
- `and rd, rs, rt`: 与
- `andi rd, rs, rt`: 与上一个立即数

- `or rd, rs, rt`: 或
- `ori rd, rs, rt`: 或上一个立即数

- `xor rd, rs, rt`: 异或
- `xori rd, rs, rt`: 异或上一个立即数
...
- 在 b 类指令中, imm 为 16 位带符号数, 而且省略了最低两位 (因为最低两位一定是 `0`), 所以能转移的指令范围为 $2^25$ 条, 即 256KB (跳转到更多地方则需要借助 `j`).
- 在 `j` 中, $PC = PC_{31..28} || instr_index || 0^2$, 则转移的范围为 256MB.
