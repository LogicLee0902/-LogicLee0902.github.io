...
浮点数(32 位) = 符号(1) + E(8) + M(23) + B(默认为 2, 省略).
...
`!=` 可以用 `==` 实现, 其他运算均可以用 `<`, 如 `A >= B` 即 `not (A < B)`.
...
试商只要试 `0` 和 `1`, 中间余数不断 `<<` 与 `|` 即可.

对于符号数, 先算绝对值再决定符号. 同号则商为正, 否则商为负. 余数的符号和被除数相同.

# 参考资料

1. Digital Design and Computer Architecture 2nd, Chapter 1 From Zero to One, Section 1.4 Number Systems
