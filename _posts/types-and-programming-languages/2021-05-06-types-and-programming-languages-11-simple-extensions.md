---
layout: "post"
title: "「TAPL」 09 Simple Extensions (Unfinished)"
...
Pattern Matching 依赖于一个 `match(p, v)` 函数，表示 value 是否匹配模式，如果匹配就产生一个替换。
...
- $\operatorname{\mathtt{match}}(\{x\}, \{5, \operatorname{\mathtt{true}}\})$ fails

`match` 由 `M-Var` 和 `M-Rcd` 两条规则定义。前者表示一个 variable 可以和任何 value 匹配，后者定义了 record 形式模式的匹配。
